...
export function createEmployeeRow(employee: Employee): HTMLDivElement {
  const { id, image, admission_date, job, name, phone } = employee;
  ...
  divRow.innerHTML = `
    ...
    <td data-head="Data de Admissão" class="date-grid details-content details-${id}">${formatDateToDDMMYYYY(admission_date)}</td>
